...
def build_context(records: list[Document]) -> str:
    ...
    for idx, record in enumerate(records, start=1):
        ...
        header = (
            f"[片段{idx}] chunk_id={metadata.get(ID_KEY, '')} "
...
def build_retrieval_debug(records: list[Document]) -> list[dict[str, str]]:
    """构建检索调试信息，输出片段来源与短预览。"""
    debug_rows: list[dict[str, str]] = []
    for record in records:
        metadata = record.metadata or {}
        preview = record.page_content.replace("\n", " ").strip()[:120]
        debug_rows.append(
            {
                ID_KEY: str(metadata.get(ID_KEY, "")),
